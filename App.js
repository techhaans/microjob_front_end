...
import { createNativeStackNavigator } from "@react-navigation/native-stack";
import AsyncStorage from "@react-native-async-storage/async-storage";
import { ActivityIndicator, View, Text, UIManager, Platform, LogBox } from "react-native";
      ...
      if (token && role) {
        switch (role) {
          case "DOER": {
            ...
            const parsedProfile = safeJSONParse(profileData);
            setInitialRoute("Dashboard");
            setInitialParams(parsedProfile?.isNew ? { showEditProfilePrompt: true } : null);
            break;
          }
          case "POSTER":
            setInitialRoute("PosterDashboard");
  ...
  if (loading) {
    return (
      <View style={{ flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: "#fff" }}>
        <ActivityIndicator size="large" color="#007bff" />
        <Text style={{ marginTop: 12, color: "#555" }}>Checking your session...</Text>
      </View>
    );
  }
