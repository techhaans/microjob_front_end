...
export default function DoerDashboard() {
  ...
  const [radius, setRadius] = useState(5);
  const [location, setLocation] = useState(null);
  const [liveTrackingEnabled, setLiveTrackingEnabled] = useState(true); // default ON
  const [loadingJobs, setLoadingJobs] = useState(false);
  const [acceptingMap, setAcceptingMap] = useState({});
  ...
  const startLocationFlow = async () => {
    try {
      ...
      let coords = null;
      if (last?.coords) {
        coords = { lat: last.coords.latitude, lon: last.coords.longitude };
      } else {
        const loc = await Location.getCurrentPositionAsync({
      ...
      startLocationWatcher(); // start live updates automatically
    } catch (err) {
      console.warn("startLocationFlow error:", err);
    }
  };
        ...
        >
          ...
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            <Text
              style={{
                marginRight: 8,
                color: liveTrackingEnabled ? "#16a34a" : "#f59e0b",
              }}
            >
      ...
      {activeTab === "available" && (
        <View style={{ flex: 1 }}>
          ...
          <FlatList
            ...
            renderItem={({ item, index }) => (
              ...
              >
                <View style={styles.cardHeader}>
                  <Text style={styles.title}>{item.title}</Text>
                  <View
                    style={[styles.statusTag, { backgroundColor: "#2563eb22" }]}
                  >
            ...
            contentContainerStyle={{ padding: 16, paddingBottom: 80 }}
            ListEmptyComponent={
              <Text style={styles.emptyText}>No nearby jobs yet.</Text>
            }
          />
        </View>
      )}
