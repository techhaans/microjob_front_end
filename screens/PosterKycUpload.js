...
export default function PosterKycUpload() {
  ...
  const handleUpload = async () => {
    if (!file) return Alert.alert("Validation", "Please choose a file first.");

    setLoading(true);
    try {
      const result = await uploadPosterKyc(file, docType);
      ...
      console.error("❌ Upload error:", err);
      Alert.alert("Error", err.message || "KYC upload failed.");
    } finally {
      setLoading(false);
    }
  };
...
const styles = StyleSheet.create({
  container: { flex: 1, padding: 16, backgroundColor: "#f2f6fb" },
  backButton: { marginBottom: 8, paddingVertical: 6 },
